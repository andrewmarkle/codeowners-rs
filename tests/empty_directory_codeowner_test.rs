...
/// `owner` is `content.trim()`, so an empty or whitespace-only `.codeowner` yields `""`
/// and hits the same silent-inheritance path as a typo'd name. Reporting it as
/// `an invalid team - ''` would give no hint the file is empty.
#[test]
